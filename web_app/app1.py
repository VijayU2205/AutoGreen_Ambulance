...
# Raspberry Pi Server URL - YOUR RASPBERRY PI IP (update if different)
...
# Receive location updates from dashboard (driver)
...
def send_location():
    ...
    location_data = {
        ...
        'ambulance_no': session.get('ambulance_no')
    }
...
# New: routing distance info from dashboard -> forward to Raspberry Pi
...
def send_routing_distance():
    ...
    try:
        # Forward to Raspberry Pi server. If your Pi expects a different endpoint name, adjust it there.
        response = requests.post(f"{RASPBERRY_PI_SERVER}/api/routing_distance", 
                               json=routing_data, timeout=5)
        # ignore response content, return success back to dashboard
...
if __name__ == '__main__':
    ...
    print("🛣️  ROUTING FEATURE (OSRM public) ENABLED")
